...
from .mongodb import create_password, delete_password, get_password, update_password
...
def create():
    if request.method == "GET":
        ...
        new_password = {
            ...
            "created_at": now,
            "expire_days": int(expirationDays),
            "expire_views": int(expirationViews),
            "viewer_deletable": allowDeletion, 
            "views": 0,
...
def preview(password_id):
    ...
    if (data is None):
        ...
        # Return the url
        origin = request.url_root.replace("http", "https")
...
def password(password_id):
    ...
    if (data["expire_on"] < now):
        ...
        fernet = Fernet(encryptionKey)
        decrypted_password = fernet.decrypt(data["password"]).decode()
